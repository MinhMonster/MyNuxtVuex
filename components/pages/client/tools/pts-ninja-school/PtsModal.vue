<template>
  <v-dialog v-model="dialog" scrollable max-width="600px" light>
    <template v-slot:activator="{ on, attrs }">
      <div v-bind="attrs" v-on="on">
        <b-button variant="primary"> {{ data.name }} </b-button>
      </div>
    </template>
    ...
    <v-card>
      ...
      <v-card-text style="height: 300px">
        <div class="page-body">
          <div class="scroll-y">
            <form>
              <div class="flex-wrap gap-15">
                <div class="field flex gap-5 text-center">
                  <form-validator name="id">
                    <label for="name" class="form-label">(+)</label>
                    <b-form-input
                      v-model="data.value"
                      :style="styleBtn"
                      class="v-input form-input input-btn"
                    ></b-form-input>
                  </form-validator>
                  <form-validator name="id">
                    ...
                    <b-form-input
                      v-model="data.color"
                      type="color"
                      class="v-input form-input input-btn btn-color"
                    ></b-form-input>
                  </form-validator>
                  <form-validator name="id">
                    <label for="name" class="form-label">Nền</label>
                    <b-form-input
                  ...
                  <b-button
                    class="mt-6 danger v-btn text-14-500"
                    @click="preData()"
                    >-</b-button
                  >

                  <b-button class="mt-6 primary v-btn" @click="nextData()"
                    >+</b-button
                  >
                </div>
                <!-- <div class="field flex gap-10">
                  <form-validator name="id">
                    <label for="name" class="form-label">T.L</label>
                    <b-form-input
                      v-model="data.tl"
                      class="v-input form-input input-btn"
                    ></b-form-input>
                  ...
                  <b-button
                    class="mt-5 danger v-btn text-14-500"
                    @click="preData()"
                    >-</b-button
                  >
                  <b-button class="mt-5 primary v-btn" @click="nextData()"
                    >+</b-button
                  >
                </div> -->
              </div>
  ...
  computed: {
    ...
    styleBtn() {
      return {
        ...
        backgroundColor: this.data.bgColor + "!important",
        border: "1px solid" + this.data.bgColor + "!important",
      };
    },
  },
  methods: {
    nextData() {
      if (this.data.value == 16) return;
      this.data.value += 1;
    },
    preData() {
